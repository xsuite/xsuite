...
\subsection{Redefinition of $\zeta$}

For the tracking of coasting beams we define the longitudinal coordinate $\zeta$ as:

In Xsuite the tracked variable is:
\begin{equation}
\zeta = s  - \beta_0 c (T - k\Delta T)
\end{equation}
where $s$ is the distance from the start of the line for the present turn, $T$ is the absolute time since the start of the simulation, $k$ is the number of timew the particle has passed at the end of the line.
...
\begin{align}
\zeta^- &= L - \beta_0 c (T - k\Delta T)\\
\zeta^+ &= 0 - \beta_0 c (T - (k+1)\Delta T)
\end{align}
...
For coasting beams $\Delta T$ is given by Eq.\,\ref{eq:deltatdef}, hence the $\zeta$ needs to be update at each turn using:
\begin{equation}
...
\begin{equation}
t(s) = \frac{s}{\beta_0 c} + n\frac{L}{\beta_0 c} - \zeta(s) 
\label{eq:tfromzeta}
\end{equation}
...
\begin{equation}
- \frac{\Delta T}{2} < \frac{s}{\beta_0 c} + n\frac{L}{\beta_0 c} - \frac{\zeta(s)}{\beta_0 c}  - n \Delta T - \frac{s}{\beta_\text{sim} c} < \frac{\Delta T}{2}
\end{equation}

Rearraging we can write:
\begin{equation}
- \beta_0 c \frac{\Delta T}{2} < s \left(1 - \frac{\beta_0}{\beta_\text{sim} } \right) + n\left(L -\beta_0 c \Delta T\right) - \zeta(s) < \frac{\Delta T}{2}
\end{equation}

Using Eq.\,\ref{eq:deltatdef}, we can write:
\begin{equation}
- \frac{\beta_\text{sim}}{\beta_1} \frac{L}{2} < (s + nL) \left(1 - \frac{\beta_0}{\beta_\text{sim} } \right) - \zeta(s) < \frac{\beta_\text{sim}}{\beta_1} \frac{L}{2}
\end{equation}
...
Rearranging:
\begin{equation}
 -\frac{\beta_0}{\beta_\text{sim}} \frac{L}{2} 
 + s\left(1 - \frac{\beta_0}{\beta_\text{sim} } \right)
 <  \zeta(s) 
 - nL \left(1 - \frac{\beta_0}{\beta_\text{sim} } \right)
 < \frac{\beta_\text{sim}}{\beta_1} \frac{L}{2}
  + s\left(1 - \frac{\beta_0}{\beta_\text{sim} } \right)
\end{equation}
