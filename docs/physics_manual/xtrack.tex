...
Additional information can be found in \cite{silke_cap}.

\begin{align*}
x_f &= x + \frac{1}{2} \frac{\partial \psi}{\partial p_x} y_f^2 \\
p_{x,f} &= p_x \\
y_f &= \frac{2y}{1 + \sqrt{1 - 2 \frac{\partial \psi}{\partial p_y} y}} \\
p_{y,f} &= p_y - \psi y_f - \frac{b_0^2}{9 K g (1 + \delta)} y_f^3 \\
...
- g b_0 K \left( 1 + \frac{p_x^2}{p_s^2} \left( 2 + \frac{p_y^2}{p_s^2} \right) \right) p_s 
\right]
\end{equation}
where $b_0$ is the normalized magnetic field, $g$ is the magnet gap and $K$ is
the fringe field integral:
\begin{equation}
K = \int_{-\infty}^{+\infty} \frac{b(s)\bigl(b_0 - b(s)\bigr)}{g b_0^{2}} \, ds
\end{equation}

\subsection{Dipole wedge}
...
\begin{align}
x(\theta) &= x \cos\theta 
+ \frac{ x p_x \sin(2\theta) + \sin^2\theta \left( 2x \sqrt{(1+\delta)^2 - p_x^2 - p_y^2} - b_0 x^2 \right) }
{\sqrt{(1+\delta)^2 - p_x(\theta)^2 - p_y^2} + \sqrt{(1+\delta)^2 - p_x^2 - p_y^2} \cos\theta - p_x \sin\theta} 
\\[1ex]
p_x(\theta) &= p_x \cos\theta + \left( \sqrt{(1+\delta)^2 - p_x^2 - p_y^2} - b_0 x \right) \sin\theta
\\[1ex]
y(\theta) &= y + \frac{p_y}{b_0} \theta 
+ \frac{p_y}{b_0} \left( 
...
\delta(\theta) &= \delta
\\[1ex]
\ell(\theta) &= \ell + \frac{(1+\delta)}{b_0} \theta
+ \frac{(1+\delta)}{b_0} \left(
\sin^{-1}\!\left( \frac{p_x}{\sqrt{(1+\delta)^2 - p_y^2}} \right)
- \sin^{-1}\!\left( \frac{p_x(\theta)}{\sqrt{(1+\delta)^2 - p_y^2}} \right)
\right)
\end{align}

where $b_0$ is the normalized magnetic field.

\subsection{Y rotation}

The following map can be used to move the observation plane by an angle $\theta$
around the $y$ axis in the absence of a magnetic field~\cite{forest99}.

\begin{align}
x^{\mathrm{new}} &=
\frac{x}{\cos\theta \left( 1 - \frac{p_x \tan\theta}{p_z} \right)}
\\[1ex]
p_x^{\mathrm{new}} &=
p_x \cos\theta + p_z \sin\theta
\\[1ex]
y^{\mathrm{new}} &=
y + \frac{p_y x \tan\theta}{p_z \left( 1 - \frac{p_x \tan\theta}{p_z} \right)}
\\[1ex]
p_y^{\mathrm{new}} &= p_y
\\[1ex]
\ell^{\mathrm{new}} &=
\ell + \frac{(1+\delta) x \tan\theta}{p_z \left( 1 - \frac{p_x \tan\theta}{p_z} \right)}
\end{align}
where
\begin{equation}
p_z = \sqrt{(1+\delta)^2 - p_x^2 - p_y^2}.
...
\begin{align}
p_x^f &= p_x - b_2 x^2 \theta + b_2 \frac{y^2}{2} \theta 
\label{eq:pxf}
\\
p_y^f &= p_y + b_2 x y \theta
\label{eq:pyf}
\end{align}

\subsubsection*{Derivation}
(by S. Van der Schueren)
...
\left( 
\sqrt{(1+\delta)^2 - p_x^2 - p_y^2} 
- \frac{x + \frac{h}{2} x^2}{1 + h x} b_1 + b_2 \frac{y^2}{2} - \frac{b_2 h x^3}{3} + \frac{b_2 x^2}{2(hx+1)}
\right)
\label{eq:Htau}
\end{equation}
which we can split in perturbation theory into:
\begin{align}
H_1 &= \frac{p_\tau}{\beta_0} - (1 + h x)\sqrt{(1+\delta)^2 - p_x^2 - p_y^2} + \left( x + \frac{h}{2}x^2 \right) b_1 
\label{eq:H1}
\\
H_2 &= -(1 + h x) b_2 \frac{y^2}{2} + \left( b_2 h \frac{x^3}{3} + b_2 \frac{x^2}{2} \right)
\label{eq:H2}
\end{align}

The Hamiltonian $H_1$ leads to the dipole wedge (or a dynamical rotation if $b_1 = 0$). 
The Hamiltonian $H_2$ has equations of motion:
\begin{align}
p_x' &= -\frac{\partial H_2}{\partial x} 
= h b_2 \frac{y^2}{2} - h b_2 x^2 - b_2 x 
\label{eq:pxprime}
\\
...
\begin{align}
p_x(s) &= p_x + h b_2 \frac{y^2}{2} s - h b_2 x^2 s - b_2 x s 
\label{eq:pxs}
\\
p_y(s) &= p_y + (1 + h x) b_2 y s 
\label{eq:pys}
\end{align}
