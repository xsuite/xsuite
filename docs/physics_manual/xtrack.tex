...
We call $\bm{e}_1, \ldots, \bm{e}_8$ the eigenvectors and $\lambda_1, \ldots, \lambda_8$ the eigenvalues so that
\begin{equation}
...
+ \mathbf{N} \bm{z}_{\text{orb}}
\end{equation}

In the following the constant term will be omitted.
...
\begin{equation}
\mathbf{A} \bm{e}_1^{\text{orb}} = \mathbf{D} \bm{e}_1^{\text{orb}} + \mathbf{A} \mathbf{N} \bm{e}_1^{\text{orb}}
\label{eq:isf_e1}
\end{equation}
...
Combining this result for the three orbital eigenvectors:
\begin{equation}
\left( \mathbf{N} \bm{e}_1^{\text{orb}} \quad \mathbf{N} \bm{e}_2^{\text{orb}} \quad \mathbf{N} \bm{e}_3^{\text{orb}} \right)
= 
\left( \bm{e}_1^{\text{spin}} \quad \bm{e}_2^{\text{spin}} \quad \bm{e}_3^{\text{spin}} \right)
\end{equation}
...
This section is based on \cite{Chao:1980fz, Derbenev:1973ia, Boge:1994we}.

The equilibrium polarization and the polarization time can be computed as:
...
\begin{equation}
\tau_\text{pol}^{-1} = \frac{5\sqrt{3}}{8} \frac{r_e \hbar \gamma^5}{m_e} \cdot \alpha_+
\end{equation}
...
\left[
1 - \frac{2}{9} (\bm{n} \cdot \hat{\bm{i}}_v)^2 + \frac{11}{18} \left|  \frac{\partial \bm{n}}{\partial \delta} \right|^2
\right]_s\\
&\alpha_- \simeq  \frac{1}{C} \oint \frac{ds}{|\rho(s)|^3} 
\left[\hat{\bm{i}}_B \cdot
(\bm{n} - \frac{\partial \bm{n}}{\partial \gamma})
\right]_s
\end{align}
