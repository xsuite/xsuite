...
\end{align}
where $\chi_{p_qj}=\arg(W_{p_q,2j-1}+\ii W_{p_q,2j})$.  Therefore the spectral amplitudes are
\begin{equation}
 A_{qj}=\sqrt{2J_j\beta_{qj}},
 \qquad
 A_{p_qj}=\sqrt{2J_j\gamma_{qj}}.
...
\begin{equation}
 ...
 \label{eq:Gq}
\end{equation}
maps physical coordinates in plane $q$ to Edwards--Teng normalized
coordinates:
\begin{equation}
 \begin{pmatrix}\widetilde q\\\widetilde p_q\end{pmatrix}
 =G_q\begin{pmatrix}q\\p_q\end{pmatrix}.
...
Since each decoupled modal map is normalized by $A_q$, the full normal-form
matrix is
\begin{equation}
 W=T_{\mathrm{ET}}\diag(A_x,A_y).
 \label{eq:w-from-tet}
\end{equation}
It is useful to express the same matrix using the ET-normalized coupling
matrix $\mathcal C$.  Define
\begin{equation}
 \boxed{
 \mathcal C=\rho G_xR_{\mathrm{ET}}^{\sharp}A_y.
 }
 \label{eq:c-from-ret}
\end{equation}
Then \cref{eq:w-from-tet} becomes
\begin{equation}
 \boxed{
 W=
...
\begin{equation}
 \boxed{
 G_xW_{x1}=\rho I_2,\qquad
 G_xW_{x2}=\mathcal C,\qquad
 G_yW_{y1}=-\mathcal C^{\sharp},\qquad
 G_yW_{y2}=\rho I_2.
 }
 \label{eq:w-blocks-normalized}
\end{equation}
Thus $\mathcal C$ is obtained directly by applying the ET normalization to a
cross-plane block of $W$.
...
Taking the quadratic projections of \cref{eq:w-factorization} gives
\begin{align}
...
\begin{tabular}{@{}lll@{}}
\toprule
Observable & Cross-mode complex coefficient & Half lattice-amplitude ratio\\
\midrule
$x$ from mode 2 & $f-g^{*}$ & $|f-g^{*}|$\\
$\widetilde p_x$ from mode 2 & $f+g^{*}$ & $|f+g^{*}|$\\
$y$ from mode 1 & $(f-g)^{*}$ & $|f-g|$\\
$\widetilde p_y$ from mode 1 & $(f+g)^{*}$ & $|f+g|$\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{align}
 \frac{A_{p_x}(Q_2)}{A_{p_x}(Q_1)}
 &=\sqrt{\frac{J_2}{J_1}}
   \sqrt{\frac{\gamma_{x2}}{\gamma_{x1}}},\\
 \frac{A_{p_y}(Q_1)}{A_{p_y}(Q_2)}
 &=\sqrt{\frac{J_1}{J_2}}
...
\subsection{Recovery of complex RDTs from MR functions}
\label{sec:recover-rdts}

The complete scalar MR set contains the phase-sensitive quadratic information
needed to reconstruct $f$ and $g$ generically.  First obtain $\rho$ from
\cref{eq:rho-from-mr} and the ET modal Twiss parameters from
\cref{eq:et-from-main-mr}.  Then construct
\begin{equation}
 X=G_x\Sigma_{x2}G_x^T=\mathcal C\mathcal C^T,
 \qquad
 Y=G_y\Sigma_{y1}G_y^T
 =\mathcal C^{\sharp}(\mathcal C^{\sharp})^T.
 \label{eq:XY-def}
\end{equation}
Then
\begin{align}
 |f|^2&=
 \frac{\Tr X+2(1-\rho^2)}{16\rho^2},
 \label{eq:fmod-from-mr}\\
 |g|^2&=
 \frac{\Tr X-2(1-\rho^2)}{16\rho^2},
 \label{eq:gmod-from-mr}\\
 fg&=
 \frac{X_{22}-X_{11}+2\ii X_{12}}{16\rho^2},
 \label{eq:fg-from-mr}\\
 fg^{*}&=
 \frac{Y_{22}-Y_{11}-2\ii Y_{12}}{16\rho^2}.
 \label{eq:fgstar-from-mr}
\end{align}
If
\begin{equation}
 P=fg,
 \qquad Q=fg^{*},
\end{equation}
then, whenever both RDTs are nonzero,
\begin{align}
 \arg f&=\frac12\left(\arg P+\arg Q\right)\pmod{\pi},\\
 \arg g&=\frac12\left(\arg P-\arg Q\right)\pmod{\pi}.
\end{align}
The scalar MR functions therefore determine $(f,g)$ up to the common transformation
\begin{equation}
 (f,g)\longrightarrow(-f,-g),
\end{equation}
which leaves all quadratic observables invariant.  The full phase-gauged $W$ matrix, continuity along $s$, or a known source sign fixes this branch.  When $g\to0$, both phase-bearing products $P$ and $Q$ vanish; reconstruction of $\arg f$ from scalar MR functions becomes singular even though $|f|$ remains well determined.
