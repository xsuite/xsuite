...
into two stable two-dimensional modal maps.  As in the Xsuite physics manual,
...
\end{equation}
Consequently, $g$ is obtained directly from the principal MR functions:
\begin{equation}
 \boxed{
 g^2
...
Next obtain $g$ from the principal horizontal MR functions and recover the
...
\cref{eq:mr-functions-from-w}.  In the implementation, the variables
\texttt{rho2} and \texttt{rho} store the quantities denoted here by $g^2$ and
$g$, while \texttt{beta\_a} and \texttt{alpha\_a} store $\beta_x$ and
$\alpha_x$.
